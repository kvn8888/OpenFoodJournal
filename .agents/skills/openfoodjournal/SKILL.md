...
User taps Scan → CameraController (AVCaptureSession) → 1-4 photos
  → Camera view uses one native continuous zoom slider, without a custom glass capsule, over AVFoundation's system-recommended range
  → CameraController prefers a virtual rear camera so AVFoundation switches constituent lenses continuously; displayed factors use displayVideoZoomFactorMultiplier so 0.5×/1× stay consistent with system UI
...
- `.github/workflows/app-store.yml` is gated by `ENABLE_APP_STORE_AUTOMATION=true`. Its unprivileged prepare job resolves and displays the exact TestFlight binary and hash-bound public notes before the protected `app-store-production` environment asks for approval. The approved job reverifies the artifact, stages the existing `VALID` build without recompiling, copies prior public metadata, applies the reviewed notes, and runs strict readiness checks. A normal push never submits; submission requires `workflow_dispatch` with `submit_for_review=true` plus environment approval.
...
- **Assistant tab (agent foundations 2026-07-20)**: persistent CloudKit-synced agentic chat with independent Gemini/OpenRouter/Azure provider selection, independent model-native/Tavily/Parallel web-research selection, Azure Sol/Terra deployments, 50k/200k/maximum token budgets, portable compaction, durable versioned sources, cancellation/recovery, and retry-safe approved writes. Read tools (journal, Food Bank, goals, Apple Health energy, calculators, `web_search`, secure `fetch_url`, and `read_conversation_source`) run silently with activity/source chips; write tools show durable Allow/Deny permission cards. Supports direct camera capture plus photo-library/PDF attachments through one provider-neutral attachment pipeline, user-message edit-and-replay, provider-generated/regenerable titles, a cross-conversation image/file library, regenerate, provider-reported usage, context meter/warnings, and the always-visible "not medical advice" footer.
...
- **AI scan UX**: direct Gemini loading overlay shows Interactions API streamed thought-summary trace, OpenRouter shows a neutral provider wait state, and the result sheet/scan page can redo the last submitted photo set. The live camera opens directly with Scan Food/Barcode/Food Label rectangular modes, a native continuous zoom slider above them using AVFoundation's system-recommended range, circular torch/library controls flanking the shutter, exit at top-left, and conditional retry at top-right. A virtual rear-camera input lets AVFoundation switch lenses without rebuilding the session, while `displayVideoZoomFactorMultiplier` keeps the visible factor aligned with system UI. The full-screen aspect-fill preview publishes its normalized visible sensor rectangle and every shutter press snapshots that crop, so review, barcode detection, and AI input contain exactly the framing visible at capture time instead of the wider uncropped still. The zoom slider sits directly over the bottom legibility gradient with no custom glass capsule; the remaining camera chrome uses dark Liquid Glass with white labels/icons and deliberately omits logos, real-time ingredient callouts, and instructional caption bubbles.
