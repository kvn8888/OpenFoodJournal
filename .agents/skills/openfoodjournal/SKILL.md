...
- **Micronutrient goals**: `MicronutrientGoalSettings` stores per-nutrient daily targets in `UserDefaults` (`goals.micronutrientOverrides`, a JSON id-to-value object). An entry replaces that nutrient's FDA Daily Value everywhere a percentage is shown; an absent entry keeps the FDA number, and only positive finite values are stored. Read it through `UserGoals.dailyValue(for:)` rather than `KnownMicronutrient.dailyValue`, which is the FDA constant. Edited in Settings via `MicronutrientGoalsEditorView`, carried in `AppSettingsRecord` backups and mirrored to `ofj_app_settings`. `UserGoals.micronutrientOverrides` is deliberately not `@AppStorage` so observers actually update.
