...
| Release Automation | GitHub `xcode-27` cloud CI; `testflight` builds once and uploads internally; manual protected external TestFlight and `app-store` workflows promote the exact tested build without rebuilding |
...
- **Internal TestFlight: version 1.4 build 11** — immutable schema-2 prerelease manifest `testflight/1.4-11`; it remains a prerelease until Apple publishes App Store version 1.4.
