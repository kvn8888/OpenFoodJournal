...
- **AI scan UX**: direct Gemini loading overlay shows Interactions API streamed thought-summary trace, OpenRouter shows a neutral provider wait state, and the result sheet/scan page can redo the last submitted photo set. The live camera opens directly with Scan Food/Barcode/Food Label rectangular modes, small 0.5x/1x/2x zoom pills above them, circular torch/library controls flanking the shutter, exit at top-left, and conditional retry at top-right. Camera chrome uses dark Liquid Glass with white labels/icons and deliberately omits logos, real-time ingredient callouts, and instructional caption bubbles.
