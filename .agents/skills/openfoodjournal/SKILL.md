...
- `.github/workflows/screenshots.yml` is optional: manual dispatch (once present on the default branch) or the `ci:screenshots` label on PRs to `testflight`/`app-store`. It builds `OpenFoodJournalScreenshots` once and exports Journal, Food Bank, Log Food, History, Assistant, and Settings in light/dark mode as PNGs plus an offline gallery and commit/hash manifest. It is not a release gate and receives no secrets. `ScreenshotFixtures.swift` contains Debug-only sample data; both `OFJ_UI_TEST_MODE=1` and `OFJ_SCREENSHOT_MODE=1` are required for fixed presentation dates and fixture setup. Test stores explicitly disable CloudKit. Ordinary UI tests retain their existing mocked Assistant scenario. Keep real service/persistence clocks on `Date`; `AppPresentationDate.now` only stabilizes visual dates in screenshot mode. See `docs/screenshots.md` for inputs, artifact retention, and extension guidance.
