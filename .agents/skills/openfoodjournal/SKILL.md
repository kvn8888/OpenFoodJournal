...
- **`TrackedContainer`** — `@Model`, weight-based container tracking. Snapshots food nutrition at creation time. Start weight → final weight → derived consumption via `consumedServings` math.
  - The start form supports either direct gross-weight entry or a tare workflow. Tare mode records the empty container plus the loaded container, persists optional `tareWeight`, keeps `startWeight` as the loaded gross measurement, and derives initial/remaining food weights without changing legacy consumption math. Completion rejects weights below a stored tare and reminds the user to use the same bowl/lid/accessories.
